...
url = "https://eredivisie.nl/competitie/stand/"
...
for row in rows[1:]:  # Sla eerste rij (koptekst) over
    ...
    if len(cells) >= 4:  # Controleer of er voldoende cellen zijn
        ...
        team3 = cells[3].text.strip()
        score = cells[1].text.strip()  # Verwacht een "score" cel
...
# Create a page
teletextPage = {"number":281,"subpages":[{"packets":copy.deepcopy(newsIndexTemplate["subpages"][0]["packets"])}]}
