...
response = requests.get('https://intern.bollenstreekmedianetwerk.nl/textbulletin2/nmptv_epg.json')
...
# Maak pagina 402 met subpagina's en cycleTime
teletextPage = {"number": 402, "subpages": [], "control": {"cycleTime": "15,T"}}
...
while program_index < len(epg_data_sorted):
    ...
    titleBlock = toTeletextBlock(
        input = {"content": [{"align": "left", "content": [{"colour": "yellow", "text": "BMN 2 TV PROGRAMMA VOOR VANDAAG"}]}]},
        line = line
    )
...
print(f"EPG pagina P402 aangemaakt met {len(teletextPage['subpages'])} subpagina's")
